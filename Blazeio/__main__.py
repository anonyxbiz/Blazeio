...
class App:
    ...
    async def fetch(app, url: str):
        ...
        headers = app.default_headers(url, {header[:header.find(":")]: header[header.find(":")+1:].strip() for header in app.headers.split(",") if ":" in header} if app.headers else app.headers)
