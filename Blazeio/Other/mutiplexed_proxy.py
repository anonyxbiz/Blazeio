...
class Transporters:
    ...
    async def puller(app, r, resp):
        ...
        if isinstance(resp, io.BlazeioClientProtocol):
            await resp.eof()
    ...
    async def no_tls_transporter(app, r: io.BlazeioProtocol, remote: str, srv: dict):
        ...
        if not (conn := srv.get("conn")):
            conn = io.Session(remote, client_protocol = BlazeioClientProtocol, connect_only = 1)
            srv["conn"] = conn

        async with conn.accept() as resp:
            r.store.telemetry.ttc = r.store.telemetry.ttfb()

            await resp.write(io.ioConf.gen_payload(r.method, r.headers, r.tail, str(srv.get("port"))))
...
class App(Sslproxy, Transporters):
    ...
    async def __main_handler__(app, r):
        ...
        if not (srv := app.hosts.get(host)) or not (remote := srv.get("remote")):
            await io.log.cyan(host)
            await io.log.cyan(app.hosts)
...
class WebhookClient:
    ...
    async def add_to_proxy(app, host: str, port: int, certfile: (None, str) = None, keyfile: (None, str) = None, hostname: str = "127.0.0.1", in_try: (int, bool) = False, **kw):
        ...
        host_data = {"remote": "http://%s:%d" % (hostname, port), "port": port, "certfile": certfile, "keyfile": keyfile}
