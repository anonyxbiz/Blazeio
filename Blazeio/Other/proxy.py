...
scope = io.ddict(server_name = "blazeio.other.proxy.localhost", parent_dir = "Blazeio_Other_Proxy", server_set = io.SharpEvent(False, io.loop))
...
class App(Sslproxy, Transporters):
    ...
    async def __main_handler__(app, r):
        ...
        if not sock and srv.server_config.enforce_https:
            raise io.Abort("Permanent Redirect", 308, io.ddict(location = "https://%s:%s%s" % (server_hostname, io.Scope.args.port, r.tail)))
